...
libwidevinecdm.so.patched: libwidevinecdm.so
	...
	@cp $< $<.new
	@$(DOCKER_BIN) run -v $(CURDIR):/root/patcher --rm $(PATCHER_IMAGE) ./src/patchelf --add-needed GLIBC_ABI_DT_RELR /root/patcher/$<.new
	@$(DOCKER_BIN) run -v $(CURDIR):/root/patcher --rm $(PATCHER_IMAGE) python3 /root/patcher/add-vernaux.py /root/patcher/$<.new /root/patcher/$@
	@$(RM) $<.new
